function logl = fbm_logl_n(obs,theta,M,n)
...
% n - an integer which indicates the scaling of the timesteps
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if M(4)==0; %If the Hurst parameter, H=0.5. Calculate the likelihood normally
	...
	F(:,1)=obs(:,1) - n * mu_emit(:,1);
	var(1)=2*sigma_n^2 + sigma_d^2*n;
	...
	for t=2:T;
		...
		F(:,t)=obs(:,t) - n * mu_emit(:,t) + sigma_n^2/var(t-1) * F(:,t-1);
	...
	gamma(1)=1/2*sigma_d^2* n^(2 * H) * ((1+1).^(2*H) -2*1.^(2*H)+(1-1).^(2*H))-sigma_n^2;
	k=2:T;
	gamma(k)=1/2*sigma_d^2* n^(2 * H) * ((k+1).^(2*H) -2*k.^(2*H)+(k-1).^(2*H));
	
	%Specify initial variation and similarity
	sigma2(1)=sigma_d^2 * n^(2 * H) + 2*sigma_n^2;
	phi_last(1)=gamma(1)/sigma2(1);
	mu_cond(:,1)= n * mu_emit(:,1);
	...
	for t=2:T;
		...
		mu_cond(1,t)= n * mu_emit(1,t)+sum(phi_last(1:t-1).*(obs(1,t-(1:t-1))- n * mu_emit(1,t-1)));
		mu_cond(2,t)=n * mu_emit(2,t)+sum(phi_last(1:t-1).*(obs(2,t-(1:t-1))- n *mu_emit(2,t-1)));
